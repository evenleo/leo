...
namespace leo
{
    ...
    public:
        ...
        void handleClient(TcpConnection::ptr conn)
        {
            ...
            while (conn->read(buffer) > 0)
            {
                ...
                std::shared_ptr<Serializer> rt = call_(funcname, sr);
        ...
        std::shared_ptr<Serializer> call_(const std::string &name, Serializer &sr)
        {
        ...
        {
            mapFunctions_[name] = std::bind(&RpcServer::callproxy<F>, this, func, std::placeholders::_1, std::placeholders::_2);
        }

        template <typename F, typename S>
        void regist(const std::string &name, F func, S *s)
        {
            mapFunctions_[name] = std::bind(&RpcServer::callproxy<F, S>, this, func, s, std::placeholders::_1, std::placeholders::_2);
        }

        template <typename F>
        void callproxy(F func, Serializer *pr, Serializer &sr)
        {
            callproxy_(func, pr, sr);
        }
        template <typename F, typename S>
        void callproxy(F func, S *s, Serializer *pr, Serializer &sr)
        {
            callproxy_(func, s, pr, sr);
        }

        // 函数指针
        template <typename R, typename... Args>
        void callproxy_(R (*func)(Args...), Serializer *pr, Serializer &sr)
        {
            callproxy_(std::function<R(Args...)>(func), pr, sr);
        }

        template <typename R, typename C, typename S, typename... Args>
        void callproxy_(R (C::*func)(Args...), S *s, Serializer *pr, Serializer &sr)
        {
        ...
        template <typename R, typename... Args>
        void callproxy_(std::function<R(Args...)> func, Serializer *pr, Serializer &sr)
        {
            using args_type = std::tuple<typename std::decay<Args>::type...>;
    ...
    private:
        std::map<std::string, std::function<void(Serializer *, Serializer &sr)>> mapFunctions_;
